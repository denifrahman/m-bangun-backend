...
class Request extends CI_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        is_login();
        $this->session->set_flashdata('menu_1', 'request');
        $this->session->set_flashdata('menu_2', 'request');
        $this->load->model('');
    }

    public function index()
    {
        $this->load->view('request/main_v');
    }
    public function page()
    {
        ...
        var_dump($data['links']);
        die;
        $this->load->view('request/main_v', $data);
    }
    ...
    public function detail()
    {
        // $nama = urldecode($this->input->get('nama')) == null ? '10' : urldecode($this->input->get('nama'));
        // $data['data'] = json_decode(request_api_get(API_GET . '/Produk/getAllByNama/' . $nama, false));
        $this->load->view('request/detail_request');
    }
}
