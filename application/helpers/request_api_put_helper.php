...
*/
if (!function_exists('request_api_put')) {

    function request_api_put($url, $data, $token=true) {
        ...
        if($token){
          ...
          $request_data = $client->put($url, ['headers' => ['Authorization' => 'Bearer '.$hasil_token->token], 'form_params' => $data]);
        }else{
          $request_data = $client->put($url, ['form_params' =>$data]);
        }
